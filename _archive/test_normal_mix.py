...
def t_episode_metrics() -> str:
    ...
    assert st["alarms_per_hour"] <= 10.0, (
        f"{st['alarms_per_hour']:.1f} alarms/hour exceeds the desensitisation threshold")
...
def run_all() -> int:
    ...
    print(f"Normal Mix test suite  ({len(MIX_FILES)} files, {len(tests)} tests)\n" + "=" * 78)
    for fn in tests:
        fn()
